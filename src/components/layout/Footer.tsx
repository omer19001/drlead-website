      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-6 gap-12 mb-12">
          ...
          <div className="lg:col-span-1">
            ...
            <p className="text-[#94A3B8] text-sm leading-relaxed mb-6">
              Leads that Convert.<br />Customers that Stay.
            </p>
            <div className="flex items-center gap-2">
              {socialLinks.map((social) => (
                <a
                  ...
                  className="w-8 h-8 rounded-lg bg-white border border-[#E2E8F0] hover:border-[#BFDBFE] hover:text-[#2563EB] flex items-center justify-center text-[#94A3B8] transition-all duration-150"
                >
                  <social.Icon size={14} />
                </a>
              ))}
          ...
          {Object.entries(footerLinks).map(([category, links]) => (
            <div key={category}>
              <h4 className="text-[#0F172A] font-semibold text-sm mb-4">{category}</h4>
        ...
        <div className="bg-white border border-[#E2E8F0] rounded-2xl p-6 mb-10">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-5">
            <div>
              <h4 className="text-[#0F172A] font-semibold text-base mb-1">Stay in the loop</h4>
              <p className="text-[#64748B] text-sm">Get the latest updates and resources, delivered weekly.</p>
            </div>
            <form className="flex gap-2 w-full md:w-auto" onSubmit={(e) => e.preventDefault()}>
              <input
                type="email"
                placeholder="Enter your email"
                className="flex-1 md:w-60 px-4 py-2.5 bg-[#F8FAFC] border border-[#E2E8F0] rounded-xl text-sm text-[#0F172A] placeholder-[#94A3B8] focus:outline-none focus:border-[#2563EB]/50 focus:bg-white transition-all"
              />
              <button
                type="submit"
                className="px-5 py-2.5 bg-[#2563EB] hover:bg-[#1D4ED8] text-white text-sm font-semibold rounded-xl transition-colors whitespace-nowrap"
              >
                Subscribe
              </button>
            </form>
        ...
        {/* Bottom bar */}
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 pt-6 border-t border-[#E2E8F0]">
          <p className="text-[#94A3B8] text-sm">© 2026 DrLead. All rights reserved.</p>
          <div className="flex items-center gap-6">
            <Link href="/privacy-policy" className="text-[#94A3B8] hover:text-[#64748B] text-sm transition-colors">
              Privacy Policy
            </Link>
            <Link href="/terms-of-service" className="text-[#94A3B8] hover:text-[#64748B] text-sm transition-colors">
              Terms of Service
            </Link>
            <span className="text-[#CBD5E1] text-sm">drlead.io</span>
          </div>
        </div>
      </div>
    </footer>
